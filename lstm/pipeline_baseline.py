...
arg_parser.add_argument('--max_epoch', default=30, type=int, help='Max epoch number')
arg_parser.add_argument('--max_seq_len', default=30, type=int, help='Max sequence length')

arg_parser.add_argument('--embedding', default=None, help='Path to the pre-trained embedding file')
...
if mode == 'train':
    ...
    for l in list(train_label_count.keys()) + list(dev_label_count.keys()) + list(test_label_count.keys()):
...
if mode == 'train':
    ...
    # Recover the model
    word_embedding = Embedding(len(token_vocab), saved_state['embedding_dim'], padding_idx=0, sparse=True, pretrain=None, vocab=token_vocab, trainable=True)
    for target_label in labels:
    ...
    for target_label in labels:
        ...
        for epoch in range(max_epoch):
            ...
            logger.info('[{}] Epoch {:<3} [{}ms]: {:.4f} | P: {:<5.2f} '
                        'R: {:<5.2f} F: {:<5.2f}{}'.format(target_label, epoch, epoch_elapsed_time, epoch_loss,
